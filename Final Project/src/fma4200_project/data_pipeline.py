...
def write_report_section(percent_df: pd.DataFrame, missingness: pd.DataFrame, descriptive_stats: pd.DataFrame) -> None:
    ...
    REPORT_SECTION_PATH.write_text("\n".join(report_lines) + "\n", encoding="utf-8")
...
def run_cleaning_pipeline() -> dict[str, object]:
    ...
    write_report_section(percent_df, missingness, descriptive_stats_pct)
